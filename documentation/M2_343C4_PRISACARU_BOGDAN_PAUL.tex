...
\subsection{Automated Testing with Postman}
A comprehensive Postman collection is provided in \texttt{tests/postman\_collection.json} to validate the system's functionality and security requirements.

The collection includes tests for:
\begin{itemize}
    \item \textbf{Authentication}: Verifying login flows for Admin, Developer, and Viewer roles using Keycloak's API.
    \item \textbf{RBAC Enforcement}: Ensuring that restricted endpoints (e.g., \texttt{/export}) are accessible only to authorized roles.
    \item \textbf{Log Ingestion}: Verifying that logs are correctly accepted by the Ingestor Service.
    \item \textbf{Search Functionality}: Validating that ingested logs are indexed and searchable via the Dashboard.
\end{itemize}

To run the tests:
\begin{enumerate}
    \item Import the \texttt{postman\_collection.json} file into Postman.
    \item Ensure the stack is running locally.
    \item Run the collection using the Postman Collection Runner.
\end{enumerate}
